module ExpensesHelper
    ...
    def save_user_expenses(params,current_user)
        index = 0
        expense=Expense.create!(name: params[:exp_name],description: params[:exp_desc],created_by: current_user.id,deactivated: 0)
        ...
        params[:friends_ids].push(current_user.id)
            params[:friends_ids].each do |friendId|
                ...
                split_amt_user=UserExpenseSpending.create!(user_id: friendId,expense_id: expense.id,shared_amount: shared_amount[index],total_amount: params[:exp_amount],expense_date: params[:expense_date],borrowed_or_lent: params[:borroed_or_lent],paid_by: paid_by,deactivated: 0)    
